...
generateGLSL :: GLSL a -> String
generateGLSL = programString . execGLSL

execGLSL :: GLSL a -> [GLSLUnit]
execGLSL = fst . runGLSL

evalGLSL :: GLSL a -> [GLSLInfo]
evalGLSL = snd . runGLSL

runGLSL :: GLSL a -> ([GLSLUnit], [GLSLInfo])
runGLSL = execWriter
...
type GLSL a = Writer ([GLSLUnit], [GLSLInfo]) a

data GLSLInfo = InInfo In | UniformInfo Uniform | OutInfo Out
    deriving (Show, Eq)

data In =
    InFloat [GLfloat] Name
  | InInt [Int] Name
  | InBool [Bool] Name
  | InVec2 [Vec2 GLfloat] Name
  | InVec3 [Vec3 GLfloat] Name
  | InVec4 [Vec4 GLfloat] Name
  -- Unlikely, but...
  | InMat4 [Mat44 GLfloat] Name
  deriving (Show, Eq)

data Uniform =
    UniformFloat GLfloat Name
  | UniformInt Int Name
  | UniformBool Bool Name
  | UniformVec2 (Vec2 GLfloat) Name
  | UniformVec3 (Vec3 GLfloat) Name
  | UniformVec4 (Vec4 GLfloat) Name
  | UniformMat4 (Mat44 GLfloat) Name
  deriving (Show, Eq)
...
class GPU (a :: Symbol) where
    type CPU a :: *
    inConstr :: SymbolProxy a -> ([CPU a] -> Name -> GLSLInfo)
    uniformConstr :: SymbolProxy a -> (CPU a -> Name -> GLSLInfo)
    outConstr :: SymbolProxy a -> (Name -> GLSLInfo)

    logIn :: SymbolProxy a -> [CPU a] -> Name -> GLSL ()
    logIn proxy values = rtell . inConstr proxy values
    logUniform :: SymbolProxy a -> CPU a -> Name -> GLSL ()
    logUniform proxy value = rtell . uniformConstr proxy value
    logOut :: SymbolProxy a -> Name -> GLSL ()
...
testProg :: [GLSLUnit]
testProg = fst $ execWriter simpleProgram

testLog :: [GLSLInfo]
testLog = snd $ execWriter simpleProgram
...
simpleProgram = do
    ...
    viewMatrix <- uniform mat4 "viewMatrix" identity
    modelMatrix <- uniform mat4 "modelMatrix" identity
    mvpMatrix <- uniform mat4 "mvpMatrix" identity
...
(#=) :: (HasString a, HasString b,
    ...
    a -> b -> GLSL ()
(#=) to from =
    ltell $ AssignStatement (getString to) (getString from)
infixr 1 #=
...
decl qualifier glType name = do
    ltell $ Decl (qualifierSymbol qualifier) (typeSymbol glType) name
    return $ Value name qualifier glType

uniform :: (KnownSymbol t, GPU t) =>
    SymbolProxy t -> String -> CPU t -> GLSL (Value "uniform" t)
...
out :: (KnownSymbol t, GPU t) =>
    SymbolProxy t -> String -> GLSL (Value "out" t)
out glType name = do
    logOut glType name
    decl (Proxy :: SymbolProxy "out") glType name

inn :: (KnownSymbol t, GPU t) =>
    SymbolProxy t -> String -> [CPU t] -> GLSL (Value "in" t)
