...
data AABB = AABB (Vec3 GLfloat) (Vec3 GLfloat) deriving (Show, Eq)
...
aabbFromPoints points@(x:y:z:_) =
    aabbFromPointsAccum points (x :. y :. z :. ()) (x :. y :. z :. ())
aabbFromPoints _ =
...
aabbFromPointsAccum (x:y:z:rest) (miX :. miY :. miZ :. ())
                                 (maX :. maY :. maZ :. ()) =
    aabbFromPointsAccum rest
        (min miX x :. min miY y :. min miZ z :. ())
        (max maX x :. max maY y :. max maZ z :. ())
...
aabbByFace (x1:y1:z1:x2:y2:z2:x3:y3:z3:rest) =
    let minVec = min3 x1 x2 x3 :. min3 y1 y2 y3 :. min3 z1 z2 z3 :. ()
        maxVec = max3 x1 x2 x3 :. max3 y1 y2 y3 :. max3 z1 z2 z3 :. ()
